# 프로젝트 지식 목차

## 파일별 요약
| 파일 | 항목 수 | 최종 업데이트 |
|------|--------|------------|
| architecture.md | 2 | 2026-06-15 |
| errors.md | 1 | 2026-06-15 |
| conventions.md | 0 | - |
| decisions.md | 4 | 2026-06-15 |
| lessons.md | 1 | 2026-06-15 |

## 최근 추가된 지식 (최근 5건)
- [2026-06-15] decisions: A-1 좌표정합 방식 A(앵커 정합) 확정 — 시험렌더 비교로 검증
- [2026-06-15] decisions: 출력 다페이지 1PDF(사이즈=페이지) 확정
- [2026-06-15] architecture: 좌표계 3종(디자인/패턴/대지) + preset→engine 매핑 경로
- [2026-06-15] lessons: 코워크(외부 환경) 산출물 인수 체크리스트 (의존성 재현+인코딩+git)
- [2026-06-15] errors: 한글 Windows cp949 콘솔 UnicodeEncodeError (stdout UTF-8 고정으로 해결)
