...
### [2026-07-07] 등록패턴 즐겨찾기+자동분류+메타저장: 프론트 직접 Supabase(RLS admin쓰기)·키워드파생 분류·메타만 저장(파일영속화 별도Phase)
- **분류**: decision
- **발견자**: planner-architect
- **내용**: 등록패턴을 카테고리로 묶고 관리자 즐겨찾기(별표)를 전직원 공유. **결정 5가지.** **(1) 읽기/쓰기 아키텍처 = 프론트 직접 Supabase(방안 a) 채택, 백엔드 프록시(b) 기각.** 근거: 백엔드 키리스 불변제약(service_role 레포·백엔드 금지)+"webapp/static만 변경". login.html/계정메뉴가 이미 `window.supabase.createClient(url, PUBLISHABLE)`로 세션 보유(SDK가 localStorage에 세션 지속→타 페이지 createClient가 자동 승계). 이 클라가 로그인 사용자 JWT를 PostgREST에 자동 부착 → 프론트가 `supa.from('pattern_meta').select()/.upsert()` 직접 호출, **RLS가 권한 강제**. 백엔드 무변경·엔진 무변경·신규 비밀키 0. (b)프록시는 api.py에 토큰 릴레이 엔드포인트 신설=코드/표면 증가라 기각. **(2) RLS = SELECT 전체 로그인 / 쓰기 admin만.** `alter table enable row level security` 후: SELECT `to authenticated using(true)`(로그인 전원 열람), INSERT `with check ((auth.jwt()->'app_metadata'->>'role')='admin')`, UPDATE/DELETE `using(...)+with check(...)` 동일 admin식. **app_metadata는 Supabase access token JWT에 포함되는 표준 클레임**(user가 못 고치는 관리자 메타)이라 `auth.jwt()->'app_metadata'->>'role'`로 안전 접근 → auth.py의 introspection app_metadata.role='admin' 판정과 정합. anon(미로그인)은 SELECT 정책 밖이라 자동 차단(배포는 전원 로그인이라 무영향). **(3) 스키마 = 단일표 pattern_meta.** `{pattern_id text PK(=data/patterns 폴더명=/api/patterns의 id), is_favorite bool not null default false, category text(관리자 override·null이면 프론트 파생값 사용), updated_by uuid, updated_at timestamptz default now()}`. favorites/categories 분리 대신 1표=최단순. pattern_id가 안정키라 재배포로 패턴 소실→같은 이름 재등록 시 메타 자동 재부착. **(4) 자동분류 = 키워드 사전 파생(프론트), Drive경로 캡처는 후속.** 패턴명에 옷종류 키워드(농구·야구·배구·축구·핸드볼·슈팅저지·라운드반팔·긴팔·하의·반바지…) 매칭→category, 무매칭=미분류. **유효 카테고리 = Supabase override ?? 파생값.** 근거: 기존 2패턴(농구_U넥/농구_V넥)은 from-drive 이전 등록이라 Drive 경로 없음 → 이름파생 "농구"로 **마이그레이션 0·즉시 커버**. 팀접두(연세대_농구_)도 키워드 스캔이라 견딤. 관리자가 틀린 분류를 UI에서 override(Supabase 저장). **옵션 (a)from-drive 등록 시 최상위 Drive폴더명(0.농구유니폼→농구) 캡처는 정확도 최고이나 create_pattern/from-drive(등록로직) 수정 수반** → 불변제약 "등록 무변경"과 충돌하므로 **별도 후속 단계로 미룸**(그때 preset.json 또는 메타에 캡처). **(5) 등록패턴 파일 영속화 = 이번 범위 제외(별도 Phase B).** 실측: preset 폴더가 작음(농구_U넥 20KB·농구_V넥 88KB=SVG 12개 ~4KB정규화 path + number_glyphs.json 18KB). Supabase Storage 백업은 기술적으로 가능하나 백엔드에 등록시 업로드/부팅시 복원 로직 추가=표면·리스크 증가+키리스 업로드 경로 설계 필요. 이번 기능(즐겨찾기·분류)은 **메타(tiny K-V)만 필요**하므로 우선 출시. 재배포 소실 대비책=from-drive 재등록(Drive=원본 진실), 메타는 Supabase에 독립 생존→동명 재등록 시 재부착. Storage 영속화는 승인된 별도 단위로. **읽기/쓰기 흐름**: 로드시 ①GET /api/patterns(백엔드 등록패턴) ②Supabase SELECT pattern_meta 병합(effective category=override??파생, is_favorite) ③카테고리 그룹/필터 렌더. 쓰기(별표·분류)=admin UI 노출(프론트 `user.app_metadata.role==='admin'` 판정)+RLS 이중방어. **폴백**: Supabase 실패/로컬 미설정=파생카테고리+별표없음으로 degrade(백엔드 목록은 정상). **화면**: patterns.html=카테고리 칩 필터/그룹+카드 별표(admin 토글·전원 표시)+즐겨찾기만보기, work.html step1=즐겨찾기 상단+카테고리 필터(선택흐름·stepper 무변경, 추가만). **불변**: webapp/static만·엔진/인증/등록 무변경·빌드0·var(--*)·Material Symbols(star/star_border). **리스크**: RLS 오설정=권한구멍(비admin 쓰기 차단·admin 쓰기 허용·미로그인 차단 3검증 필수)·오분류(override로 교정·미분류 버킷 노출)·Supabase 장애(degrade 폴백)·마이그레이션(불필요, 파생 즉시동작). **대안 기각**: 백엔드 프록시(키리스·표면), favorites/categories 2표(과설계), Drive경로 캡처 즉시(등록로직 변경), 파일 Storage 영속화 동시(범위·리스크).
- **참조횟수**: 0
