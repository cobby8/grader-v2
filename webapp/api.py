...
# 완성본·글리프셋 '선택 후보'로 볼 확장자(대소문자 무시).
#   왜: 완성본/글리프셋 파일은 파일명에 사이즈 토큰이 없어(예: '..._완성본_본체포함.ai') 사이즈 후보에서
#   탈락한다. 그런 .ai/.pdf 를 화면 드롭다운에서 고를 수 있도록 id 와 함께 'others' 로 따로 모은다.
#   (사이즈별 등록 파일 _PATTERN_FILE_EXTS 와 목적이 다름 — 여기선 .svg 는 제외. 완성본/글리프셋은 대개 .ai)
_EXTRA_FILE_EXTS = {".ai", ".pdf"}
...
def _scan_folder_pattern_files(fid: str):
    ...
    for it in children:
        ...
        if not size:
            ...
            # ★추가(기존 warnings 는 그대로 유지)★: 사이즈가 없는 .ai/.pdf 는 '완성본/글리프셋 후보'로도
            #   함께 담는다. warnings 는 사람이 눈으로 보는 '제외 사유'일 뿐 id 가 없어 화면에서 고를 수
            #   없으므로, 여기 others 에 id 를 실어 드롭다운에서 선택 가능하게 한다(사이즈 인식 로직은 무변경).
            if ext in _EXTRA_FILE_EXTS:
                others.append({
                    "id": it.get("id"),
                    "name": name,
...
def drive_pattern_files(folder_id: str) -> JSONResponse:
    ...
    try:
        files, warnings, others = _scan_folder_pattern_files(fid)
    except gdrive.DriveConfigError as e:
        return JSONResponse(status_code=500, content={"error": f"Drive 설정 오류: {e}"})
    except gdrive.DriveError as e:
        return JSONResponse(status_code=502, content={"error": f"Drive 접근 실패: {e}"})
    ...
    return JSONResponse(content={
        ...
        "files": files,
        "warnings": warnings,
        # others = 사이즈 없는 .ai/.pdf(완성본·글리프셋 후보). 화면 드롭다운이 이 목록으로 선택지를 만든다.
        "others": others,
    })
...
async def create_pattern_from_drive(request: Request, payload: Dict[str, Any] = Body(...)) -> Any:
    ...
    #    여기서는 others(완성본/글리프셋 후보)를 쓰지 않는다 — 완성본/글리프셋은 payload 의
    #    reference_file_id/glyph_file_id 로 화면이 직접 지정하기 때문(스캔 재추정 불필요).
    try:
        files, scan_warnings, _others = _scan_folder_pattern_files(folder_id)
    except gdrive.DriveConfigError as e:
        return JSONResponse(status_code=500, content={"error": f"Drive 설정 오류: {e}"})
    except gdrive.DriveError as e:
        return JSONResponse(status_code=502, content={"error": f"Drive 접근 실패: {e}"})
